...
namespace ArchiMetrics.Analysis.Metrics
{
	...
	internal class MemberDocumentationFactory : IAsyncFactory<ISymbol, IMemberDocumentation>
	{
		...
		public Task<IMemberDocumentation> Create(ISymbol parameter, CancellationToken cancellationToken)
		{
			...
			var parameters = GetParameters(parameter as IMethodSymbol, parameterElements);
			...
			return
				symbol.Parameters.Select(
					...
					new ParameterDocumentation(
						...
						parameterDocumentations.ContainsKey(x.Name) ? parameterDocumentations[x.Name] : string.Empty))
						.ToArray();
		}
